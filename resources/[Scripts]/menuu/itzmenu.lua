...
RegisterCommand("VRP", function(source, args, rawCommand)
...
RegisterCommand("POLICE", function(source, args, rawCommand)
    ...
    SendNUIMessage({
        type = 'init',
        data = subMenuConfigs["POLICE"].data,
        resourceName = GetCurrentResourceName()
    })
    SetNuiFocus(true, true)
end, false)
...
RegisterCommand("ADMIN", function(source, args, rawCommand)
    ...
    SendNUIMessage({
        type = 'init',
        data = subMenuConfigs["ADMIN"].data,
        resourceName = GetCurrentResourceName()
    })
    SetNuiFocus(true, true)
end, false)
...
RegisterCommand("GANG", function(source, args, rawCommand)
    ...
    SendNUIMessage({
        type = 'init',
        data = subMenuConfigs["GANG"].data,
        resourceName = GetCurrentResourceName()
    })
    SetNuiFocus(true, true)
end, false)

RegisterCommand("GANG1", function(source, args, rawCommand)
    -- Wait for next frame just to be safe
    Citizen.Wait(0)
    ...
    SendNUIMessage({
        type = 'init',
        data = subMenuConfigs["GANG1"].data,
        resourceName = GetCurrentResourceName()
    })
    SetNuiFocus(true, true)
end, false)
...
RegisterCommand('closemenu', function(player, args, rawCommand)
-- Clear focus and destroy UI
    showMenu = false
    SetNuiFocus(false, false)
    SendNUIMessage({
