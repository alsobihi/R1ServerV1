...
function tvRP.addMarker(x,y,z,sx,sy,sz,r,g,b,a,visible_distance)
  local marker = {id=1,x=x,y=y,z=z,rx=0.0,ry=0.0,rz=0.0,sx=sx,sy=sy,sz=sz,r=r,g=g,b=b,a=a,visible_distance=visible_distance,rot=0,facec=0,bobud=0}
...
function tvRP.addCustomMarker(id,x,y,z,rx,ry,rz,sx,sy,sz,r,g,b,a,visible_distance,rot,facec,bobud)
  ...
  if marker.id == nil then marker.id = 0 end
...
Citizen.CreateThread(function()
  while true do
    ...
    for k,v in pairs(markers) do
      -- check visibility
      if GetDistanceBetweenCoords(v.x,v.y,v.z,px,py,pz,true) <= v.visible_distance then
        DrawMarker(MarkerTypeUpsideDownCone, -630.82,-229.72,39.50, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 192, 3, 255, true, true, 0,0)
      end
    end
  end
end)
Citizen.CreateThread(function()
  while true do
    ...
    for k,v in pairs(markers) do
      -- check visibility
      if GetDistanceBetweenCoords(v.x,v.y,v.z,px,py,pz,true) <= v.visible_distance then
        DrawMarker(MarkerTypeUpsideDownCone, -31.27,-1239.30,31.33, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 0,255,231, true, true, 0,0)
      end
    end
  end
end)
Citizen.CreateThread(function()
  while true do
    ...
    for k,v in pairs(markers) do
      -- check visibility
      if GetDistanceBetweenCoords(v.x,v.y,v.z,px,py,pz,true) <= v.visible_distance then
        DrawMarker(MarkerTypeUpsideDownCone, 130.63,-1189.13,31.50, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.5001, 1555, 0,255,231, true, true, 0,0)
      end
    end
  end
end)
