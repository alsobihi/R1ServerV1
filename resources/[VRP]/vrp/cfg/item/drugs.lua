...
local pills_choices = {}
pills_choices["اخذ"] = {function(player,choice)
...
local smoke_choices = {}
smoke_choices["اخذ"] = {function(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil then
...
local smell_choices = {}
smell_choices["اخذ"] = {function(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil then
...
local lsd_choices = {}
lsd_choices["اخذ"] = {function(player,choice)
  local user_id = vRP.getUserId(player)
  if user_id ~= nil then
...
items["pills"] = {"بندول","A simple medication.",function(args) return pills_choices end,0.1}
items["weed"] = {"حشيش","A some weed.",function(args) return smoke_choices end,0.10}
items["cocaine"] = {"ميث","Some cocaine.",function(args) return smell_choices end,0.5}
items["lsd"] = {"كيبتاجون","Some LSD.",function(args) return lsd_choices end,0.1}
items["Medical Weed"] = {"حشيش طبي","Used by Doctors."}
items["Presents"] = {"طرد","Given to Children."}
