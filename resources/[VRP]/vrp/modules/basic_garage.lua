...
for group,vehicles in pairs(vehicle_groups) do
        ...
        if vname then
          ...
          if vehicle and vRP.tryFullPayment(user_id,vehicle[2]) then
            ...
            vRPclient.notify(player,{lang.money.paid({vehicle[2]})})
				TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='black'>تم شراء السياره بنجاح.</span>", type = "success", timeout = (3000),layout = "centerRight"})
            vRP.closeMenu(player)
          else
  ...
  menu[lang.garage.sell.title()] = {function(player,choice)
    ...
    if user_id ~= nil then
      ...
      local choose = function(player, choice)
		vRP.prompt(player,"سبب البيع؟","",function(player, msg)
        if string.len(msg) > 1 and string.len(msg) < 100 then
		local vname = kitems[choice]   	
        if vname then
          -- sell vehicle
          local vehicle = vehicles[vname]
...
RegisterCommand('ccheckc', function(player, args, rawCommand)
