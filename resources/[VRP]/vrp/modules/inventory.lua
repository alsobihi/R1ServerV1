    ...
    if user_id ~= nil then
      ...
      vRP.prompt(player,lang.inventory.trash.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
        ...
        if vRP.tryGetInventoryItem(user_id,idname,amount,false) then
          TriggerClientEvent("pNotify:SendNotification",player,{text = "حذف <span color='green'>"..vRP.getItemName(idname).."</span> "..amount, type = "info", timeout = (3000),layout = "centerRight"})
          vRPclient.playAnim(player,{true,{{"pickup_object","pickup_low",1}},false})
        else
          TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>قيمة غير صالحة</span>", type = "error", timeout = (3000),layout = "centerRight"})
        end
      end)
    end
  end
          ...
          vRP.prompt(player,lang.inventory.give.prompt({vRP.getInventoryItemAmount(user_id,idname)}),"",function(player,amount)
            ...
            if new_weight <= vRP.getInventoryMaxWeight(nuser_id) then
              if vRP.tryGetInventoryItem(user_id,idname,amount,true) then
                ...
                PerformHttpRequest('', function(err, text, headers) end, 'POST', json.encode({username = "", content = user_id.. " Gave " ..amount.." of " ..idname.. " to " ..nuser_id}), { ['Content-Type'] = 'application/json' })
              else
                TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>قيمة غير صالحة</span>", type = "error", timeout = (3000),layout = "centerRight"})
              end
            else
              TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>الحقيبة ممتلئة</span>", type = "error", timeout = (3000),layout = "centerRight"})
            end
          end)
        else
          TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>لا يوجد لاعبين بالقرب منك.</span>", type = "error", timeout = (3000),layout = "centerRight"})
        end
      else
        TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>لا يوجد لاعبين بالقرب منك.</span>", type = "error", timeout = (3000),layout = "centerRight"})
      end
...
function vRP.giveInventoryItem(user_id,idname,amount,notify)
  ...
  if data and amount > 0 then
    ...
    if notify then
      ...
      if player ~= nil then
        TriggerClientEvent("pNotify:SendNotification",player,{text = "تلقيت <span color='green'>"..vRP.getItemName(idname).."</span> "..amount, type = "info", timeout = (3000),layout = "centerRight"})
      end
    end
  end
end
  ...
  if data and amount > 0 then
    ...
    if entry and entry.amount >= amount then -- add to entry
      ...
      if notify then
        local player = vRP.getUserSource(user_id)
        if player ~= nil then
          TriggerClientEvent("pNotify:SendNotification",player,{text = "ارسلت <span color='green'>"..vRP.getItemName(idname).."</span> "..amount, type = "info", timeout = (3000),layout = "centerRight"})
        end
      end
  ...
  if user_id ~= nil then
    ...
    if data.inventory ~= nil then
      if not chests[name] then
            ...
            vRP.prompt(source, lang.inventory.chest.take.prompt({citem.amount}), "", function(player, amount)
              ...
              if amount >= 0 and amount <= citem.amount then
                ...
                if new_weight <= vRP.getInventoryMaxWeight(user_id) then
                 ...
                 TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>الحقيبة ممتلئة</span>", type = "info", timeout = (3000),layout = "centerRight"})
                end
              else
                TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>قيمة غير صالحة</span>", type = "error", timeout = (3000),layout = "centerRight"})
              end
          ...
          local cb_put = function(idname)
            vRP.prompt(source, lang.inventory.chest.put.prompt({vRP.getInventoryItemAmount(user_id, idname)}), "", function(player, amount)
              ...
              if new_weight <= max_weight then
                ...
                TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>ممتلى</span>", type = "info", timeout = (3000),layout = "centerRight"})
              end
            end)
          end
        ...
        TriggerClientEvent("pNotify:SendNotification",player,{text = "<span color='red'>تم الفتح من قبل شخص اخر.</span>", type = "info", timeout = (3000),layout = "centerRight"})
      end
    end
  end
end
