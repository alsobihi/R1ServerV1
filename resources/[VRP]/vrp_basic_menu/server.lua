...
local webhook55 = "https://discordapp.com/api/webhooks/632937702572032022/T9FnB6DUGDaq2x9-TQk2eg09f-omGCoKZYyEfZ0Au5szKxUConvSdh9Qv8fKod3I6Vtn" -- حق التلويت

function sendWebhookMessage(webhook55,message)
	if webhook55 ~= "none" then
		PerformHttpRequest(webhook55, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
end

local webhook66 = "https://discordapp.com/api/webhooks/632937486569701391/6J3fJuGNzNa2asWbeYHl_hUIgt6aTvTOsupgscX4x2o3lOMJGf7D87AWLfpKTc1u05Iu" -- السجن والغرامات

function sendWebhookMessage(webhook66,message)
	if webhook66 ~= "none" then
		PerformHttpRequest(webhook66, function(err, text, headers) end, 'POST', json.encode({content = message}), { ['Content-Type'] = 'application/json' })
	end
end
...
police_weapons["Equip"] = {function(player,choice)
    vRPclient.giveWeapons(player,{{
	  ["WEAPON_COMBATPISTOL"] = {ammo=2000},
	  ["WEAPON_PUMPSHOTGUN"] = {ammo=2000},
	  ["WEAPON_NIGHTSTICK"] = {ammo=2000},
	  ["WEAPON_STUNGUN"] = {ammo=2000}
	}, true})
      ...
      if nuser_id ~= nil then
        vRPclient.isInComa(nplayer,{}, function(in_coma)
          if in_coma then
			...
			vRPbm.logInfoToFile("LootLog.txt",user_id .. " looted " .. nuser_id .." tookmoney: " .. vRP.getMoney({nuser_id}) .. "\r\n")
	     sendWebhookMessage(webhook55,"```xl\r\n User_id " .. user_id .." looted " .. nuser_id .." and took money "..  vRP.getMoney({nuser_id}) .."```")
          else
	...
	if user_list ~= "" then
	  vRP.prompt({player,"Players Nearby:" .. user_list,"",function(player,target_id) 
	    if target_id ~= nil and target_id ~= "" then 
			  ...
			  if target ~= nil then
                ...
                vRPclient.isHandcuffed(target,{}, function(handcuffed)  
                  if handcuffed then 
				    ...
				    vRPclient.teleport(target,{1793.3038330078,2483.2529296875,-122.70077514648}) -- teleport to inside jail
	     ...
	     sendWebhookMessage(webhook66,"```xl\r\n User_id " .. user_id .." jailed " .. target_id .." for "..  jail_time .."```")
			      else
	...
	vRP.prompt({player,"Player ID:","",function(player,target_id) 
	  if target_id ~= nil and target_id ~= "" then 
		vRP.getUData({tonumber(target_id),"vRP:jail:time",function(value)
	     ...
	     sendWebhookMessage(webhook66,"```xl\r\n User_id " .. user_id .." unjailed " .. target_id .." from "..  custom .."```")
				else
				  vRPclient.notify(player,{"~r~That ID seems invalid."})
				end
			  else
	  ...
	  if value ~= nil then
	    ...
	    if custom ~= nil then
		  if tonumber(custom) > 0 then
            ...
            vRPclient.teleport(target,{1793.3038330078,2483.2529296875,-122.70077514648}) -- teleport inside jail
			    ...
			    if reason ~= nil and reason ~= "" then 
				  ...
				  if target ~= nil then
		            ...
		            if vRP.tryFullPayment({tonumber(target_id), tonumber(fine)}) then
					  ...
					  sendWebhookMessage(webhook66,"```xl\r\n User_id " .. user_id .." fined " .. target_id .." the amount of "..  fine .." for ".. reason"```")
                      vRP.closeMenu({player})
                    else
                      vRPclient.notify(player,{lang.money.not_enough()})
                    end
...
---------------------------------------------------------------------------
RegisterCommand('yyy', function(player, args, rawCommand)
  local user_id = vRP.getUserId({player})
  if user_id ~= nil then
...
RegisterCommand('loot', function(player, args, rawCommand)
  local user_id = vRP.getUserId({player})
  if user_id ~= nil then
    vRPclient.getNearestPlayer(player,{10},function(nplayer)
      local nuser_id = vRP.getUserId({nplayer})
...
RegisterCommand('jjail', function(player, args, rawCommand)
  vRPclient.getNearestPlayers(player,{15},function(nplayers) 
	local user_list = ""
    for k,v in pairs(nplayers) do
	  user_list = user_list .. "[" .. vRP.getUserId({k}) .. "]" .. GetPlayerName(k) .. " | "
...
RegisterCommand('ggg', function(player, args, rawCommand)
  local user_id = vRP.getUserId({player})
...
RegisterCommand('ffine', function(player, args, rawCommand)
  vRPclient.getNearestPlayers(player,{15},function(nplayers) 
	local user_list = ""
    for k,v in pairs(nplayers) do
	  user_list = user_list .. "[" .. vRP.getUserId({k}) .. "]" .. GetPlayerName(k) .. " | "
...
RegisterCommand('sspa', function(player, args, rawCommand)
  local user_id = vRP.getUserId({player})
...
-- if vRP.hasPermission({user_id,"toggle.service"}) then
  --     choices["عمليات الشرطة"] = choice_service -- toggle the receiving of missions
 --   end
...
--    choices["Mug"] = ch_mug -- steal nearest player wallet
 --   end
    ...
    if vRP.hasPermission({user_id,"hacker.hack"}) then
      choices["تهكير"] = ch_hack --  1 in 100 chance of stealing 1% of nearest player bank
    end
...
vRP.registerMenuBuilder({"admin", function(add, data)
  ...
  if user_id ~= nil then
	...
	if vRP.hasPermission({user_id,"admin.deleteveh"}) then
      choices["حذف سيارة"] = ch_deleteveh -- Delete nearest vehicle (Fixed pull request https://github.com/Sighmir/vrp_basic_menu/pull/11/files/419405349ca0ad2a215df90cfcf656e7aa0f5e9c from benjatw)
	end
	
	if vRP.hasPermission({user_id,"admin.spawnveh"}) then
      choices["اخراج سيارة"] = ch_spawnveh -- Spawn a vehicle model
	end
	
	--if vRP.hasPermission({user_id,"admin.godmode"}) then
      --choices["انعاش تلقائي"] = ch_godmode -- Toggles admin godmode (Disable the default admin.god permission to use this!) 
	--end

    if vRP.hasPermission({user_id,"player.blips"}) then
      choices["اظهار الاعبين في الخريطة"] = ch_blips -- turn on map blips and sprites
    end
...
--  end
	...
	if vRP.hasPermission({user_id,"player.tptowaypoint"}) then
      choices["الذهاب الى النقطة"] = choice_tptowaypoint -- teleport user to map blip
	end
	
--	if vRP.hasPermission({user_id,"admin.easy_unjail"}) then
--      choices["فك سجن"] = ch_unjail -- Un jails chosen player if he is jailed (Use admin.easy_unjail as permission to have this in admin menu working in non jailed players)
--    end
...
vRP.registerMenuBuilder({"police", function(add, data)
  ...
  if user_id ~= nil then
	...
	if vRP.hasPermission({user_id,"police.easy_jail"}) then
      choices["سجن"] = ch_jail -- Send a nearby handcuffed player to jail with prompt for choice and user_list
    end
	
--	if vRP.hasPermission({user_id,"police.easy_unjail"}) then
--      choices["اخراج من السجن"] = ch_unjail -- Un jails chosen player if he is jailed (Use admin.easy_unjail as permission to have this in admin menu working in non jailed players)
--    end
	
	if vRP.hasPermission({user_id,"police.easy_fine"}) then
      choices["مخالفة"] = ch_fine -- Fines closeby player
    end
	
	if vRP.hasPermission({user_id,"police.easy_cuff"}) then
      choices["كلبشة"] = ch_handcuff -- Toggle cuffs AND CLOSE MENU for nearby player
    end
	
	if vRP.hasPermission({user_id,"police.spikes"}) then
      choices["اشواك"] = ch_spikes -- Toggle spikes
    end
	
    if vRP.hasPermission({user_id,"police.drag"}) then
      choices["حمل الاعب"] = ch_drag -- Drags closest handcuffed player
    end
...
vRP.registerMenuBuilder({"phone", function(add) -- phone menu is created on server start, so it has no permissions.
    ...
    choices["دفع برصيد سوا"] = ch_mobilepay -- transfer money through phone
    choices["شحن رصيد"] = ch_mobilecharge -- charge money through phone
